...
import { Comparator, IDateFilterParams, IDoesFilterPassParams, NullComparator } from '../interfaces/filter';
...
@Component({
  ...
  template: `
    <div class="row">
      ...
      <div class="col-3">
        <fui-date-container *ngIf="isInRange()" [appendTo]="'.fui-datagrid-filters-popover'">
...
export class FuiDatagridDateFilter extends FuiDatagridBaseFilter<IDateFilterParams> implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.getFilterService()) {
      ...
      if (filter && filter instanceof FuiDatagridDateFilter) {

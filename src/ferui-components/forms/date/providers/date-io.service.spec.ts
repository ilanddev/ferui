  ...
  describe('Date IO Service', () => {
    ...
    describe('Date Processing', () => {
      ...
      it('ignores just text', () => {
        ...
        const date: Date = dateIOService.getDateValueFromDateOrString(inputDate);
        expect(date).toBeNull();
      });

      it('ignores invalid dates', () => {
        let inputDate: string = '10/21/test';
        const date1: Date = dateIOService.getDateValueFromDateOrString(inputDate);
        expect(date1).toBeNull();

        inputDate = 'test/1/1';
        const date2: Date = dateIOService.getDateValueFromDateOrString(inputDate);
        expect(date2).toBeNull();

        inputDate = 'test test test';
        const date3: Date = dateIOService.getDateValueFromDateOrString(inputDate);
      ...
      it('parse a two digit year', () => {
        ...
        let date: Date = dateIOService.getDateValueFromDateOrString(inputDate);
        expect(date).not.toBeNull();
        expect(assertEqualDates(date, new Date(2020, 0, 2))).toBe(true);

        // Invalid date with 2 digit year
        inputDate = '51/02/20';
        date = dateIOService.getDateValueFromDateOrString(inputDate);
        expect(date).toBeNull();
      });

      it('should not parse a five digit year', () => {
        const inputDate: string = '01/02/10000';
        expect(dateIOService.getDateValueFromDateOrString(inputDate)).toBeNull();
      });

...
export function FeruiIconElement() {
  'use strict';

  const _instance = (parentConstructor as any).apply(this, arguments);

  _instance.feruiIconUniqId = '_ferui_icon_' + feruiIconId;
  feruiIconId++;

  return _instance;
}

(FeruiIconElement as any).observedAttributes = ['shape', 'size', 'title'];

FeruiIconElement.prototype = Object.create(HTMLElement.prototype, {
  constructor: { configurable: true, writable: true, value: FeruiIconElement }
});

FeruiIconElement.prototype.constructor = FeruiIconElement;

FeruiIconElement.prototype._appendCustomTitle = function() {
  const cloneOffScreenSpan = <HTMLElement>offScreenSpan.cloneNode(false);
  cloneOffScreenSpan.id = this.feruiIconUniqId;
  cloneOffScreenSpan.textContent = this.currentTitleAttrVal;
  this.appendChild(cloneOffScreenSpan);
};

FeruiIconElement.prototype._setIconSize = function(size: string) {
...
FeruiIconElement.prototype.connectedCallback = function() {
  ...
  if (this.hasAttribute('shape')) {
    ...
    if (FeruiIconsApi.instance.has(this.currentShapeAttrVal)) {
      const currentShapeTemplate = FeruiIconsApi.instance.get(this.currentShapeAttrVal);
...
FeruiIconElement.prototype.attributeChangedCallback = function(
  ...
  if (attributeName === 'shape') {
    ...
    if (FeruiIconsApi.instance.has(this.currentShapeAttrVal)) {
      this.currentShapeTemplate = FeruiIconsApi.instance.get(this.currentShapeAttrVal);
    } else {
      this._injectErrorTemplate();
      return;
    }
...
FeruiIconElement.prototype.disconnectedCallback = function() {
...
FeruiIconElement.prototype._setAriaLabelledBy = function() {
  const existingAriaLabelledBy: string = this.getAttribute('aria-labelledby');
  if (!existingAriaLabelledBy) {
    this.setAttribute('aria-labelledby', this.feruiIconUniqId);
  } else if (existingAriaLabelledBy && existingAriaLabelledBy.indexOf(this.feruiIconUniqId) < 0) {
    this.setAttribute('aria-labelledby', existingAriaLabelledBy + ' ' + this.feruiIconUniqId);
  }
};

FeruiIconElement.prototype._injectTemplate = function(shapeTemplate?: string) {
...
FeruiIconElement.prototype._injectErrorTemplate = function() {
  this.currentShapeTemplate = FeruiIconsApi.instance.get('error');
  this._injectTemplate();
};
